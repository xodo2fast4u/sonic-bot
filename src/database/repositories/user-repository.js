...
export class UserRepository extends BaseRepository {
  ...
  async transferCoins(fromId, toId, amount) {
    ...
    return await this.transaction(async () => {
      await this.execute(`UPDATE users SET balance = balance - ? WHERE id = ?`, [amount, from], 'transferFrom')

      await this.execute(`UPDATE users SET balance = balance + ? WHERE id = ?`, [amount, to], 'transferTo')
