...
export default {
  cmd: ['yearly'],
  desc: 'Claim yearly reward (Requires Level 100+)',
  ...
  run: async ({ text, sonic, msg }) => {
    ...
    if (!char.isGod && char.level < REQUIRED_LEVEL) {
      return text(
        `🔒 *YEARLY REWARD LOCKED!*\nYou must reach at least *Level ${REQUIRED_LEVEL}* to claim yearly rewards!\nYour current level is *${char.level}*.`,
      );
    }

    if (!char.isGod && !(await checkEconCooldown(sonic, msg, 'yearly', COOLDOWN.YEARLY))) return;

    const base = 150000;
    const bonus = random(50000, 150000);
