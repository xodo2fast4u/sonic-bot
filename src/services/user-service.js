...
import { InsufficientFundsError, InvalidTransactionError, ValidationError } from '../core/errors.js'
...
export class UserService {
  ...
  async transferCoins(fromId, toId, amount, options = {}) {
    ...
    const [fromUser, toUser] = await Promise.all([this.getUserProfile(fromId), this.getUserProfile(toId)])
  ...
  async getUserStats(userId, options = {}) {
    ...
    if (!stats) {
      const [profile, transactions] = await Promise.all([
        ...
        includeTransactions ? this.userRepository.getTransactions(userId, transactionLimit) : Promise.resolve([]),
      ])
  ...
  async getUserActivityReport(userId, days = 7) {
    ...
    const recentTransactions = transactions.filter(t => t.timestamp >= startTime && t.timestamp <= endTime)
