  ...
  run: async ({ text, sonic }, args) => {
    ...
    try {
      ...
      if (action === 'update') {
        ...
        if (!input.includes('|'))
          return text(`${e.warn} Use: newslettermanage update <jid> <name|description> | <value>`);
      ...
      if (action === 'name') {
        ...
        if (!jid || !name) return text(`${e.warn} Use: newslettermanage name <jid> <name>`);
      ...
      if (action === 'desc') {
        ...
        if (!jid) return text(`${e.warn} Use: newslettermanage desc <jid> <description>`);
      ...
      if (action === 'picture') {
        const jid = args[1];
        const url = args[2];
        if (!jid || !url) return text(`${e.warn} Use: newslettermanage picture <jid> <url>`);
      ...
      if (action === 'removepic') {
        const jid = args[1];
        if (!jid) return text(`${e.warn} Use: newslettermanage removepic <jid>`);
      ...
      if (action === 'meta') {
        ...
        const key = args[2];
        if (!type || !key) return text(`${e.warn} Use: newslettermanage meta <invite|jid> <key>`);
      ...
      if (action === 'fetch') {
        ...
        if (!jid || !count)
          return text(`${e.warn} Use: newslettermanage fetch <jid> <count> [since] [after]`);
      ...
      if (action === 'subscribe') {
        const jid = args[1];
        if (!jid) return text(`${e.warn} Use: newslettermanage subscribe <jid>`);
