...
class CommandMetadata {
  ...
  async load() {
    ...
    try {
      ...
      throw new CommandError(`Failed to load command module ${this.modulePath}: ${error.message}`, null, {
        ...
        try {
          ...
          for (const [name, command] of commands) {
            ...
            if (!command.run || typeof command.run !== 'function') {
              issues.push(`Missing or invalid run function in ${categoryName}/${metadata.fileName}/${name}`)
            }
