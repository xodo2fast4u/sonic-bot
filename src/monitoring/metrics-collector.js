...
export class MetricsCollector extends EventEmitter {
  ...
  getAllMetrics() {
    return {
      ...
      timers: Object.fromEntries(
        Array.from(this.timers.keys()).map(key => [key, this.getTimerStats(...this.parseKey(key))])
      ),
      histograms: Object.fromEntries(
        Array.from(this.histograms.keys()).map(key => [key, this.getHistogramStats(...this.parseKey(key))])
      ),
  ...
  collectSystemMetrics() {
    ...
    if (this.lastCpuUsage) {
      const cpuPercent =
        (cpuUsage.user - this.lastCpuUsage.user + (cpuUsage.system - this.lastCpuUsage.system)) / 1000000
