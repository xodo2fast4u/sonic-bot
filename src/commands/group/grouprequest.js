...
export default {
  ...
  run: async ({ text, sonic, msg }, args) => {
    ...
    if (!action || !["list", "approve", "reject"].includes(action))
      return text(
        `${e.warn} Use: grouprequest <list|approve|reject> [numbers or mention]`,
      );

    try {
      if (action === "list") {
        const requests = await sonic.groupRequestParticipantsList(
          msg.key.remoteJid,
        );
        if (!requests.length)
          return text(`${e.check} No pending group requests.`);
      ...
      if (!participants.length)
        return text(`${e.warn} Mention or provide numbers to ${action}.`);
      ...
      await text(
        `${e.check} Request ${action}ed:\n${results
          .map((res) => `${res.jid}: ${res.status}`)
          .join("\n")}`,
      );
    } catch (err) {
      await text(
        `${e.cross} Failed to ${action} requests. ${err.message || ""}`,
      );
    }
  },
};
